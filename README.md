...
> tag. It is regenerated by the release, never by hand and never by a local build. `npm run build` writes
> into it today, so run it only to inspect the output and put `dist/` back afterwards.
