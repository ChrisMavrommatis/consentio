...
registers six tag names with no `customElements.get` guard (defect 8), so a second `new Consentio()` in one
process throws. One scenario per file means each gets a pristine document, cookie jar and custom element
registry, and every failure means what it says instead of inheriting the last test's wreckage.

## Two bootstraps

`npm test` runs everything under jsdom. `npm run test:plain` runs the cookie, saved-state and Google-signal
tests with **no browser at all** - a forty-line cookie jar and a `dataLayer`, and nothing else.
...
## `resolve.mjs` and `register.mjs`

`resolve.mjs` is loaded by both bootstraps, before anything else. It does two jobs webpack does for the bundle:

- **resolves a `./x.js` specifier to `x.ts`** (and `.mjs`/`.cjs` likewise), the way
  `resolve.extensionAlias` does in `webpack.config.js`, so the tests run the real sources rather than a
  build artifact - no build step, and stack traces point at `.ts` files
...
## Correct-behaviour tests marked `todo`

**The code under test is deliberately still broken.** Plan 1 ported it to TypeScript preserving every defect,
so the tests describe what the code *should* do and carry `{ todo: true }` where it does not yet. A failing
`todo` does not fail the run, and node:test reports one that unexpectedly passes.

**Every `todo` names its number from `chartroom/consentio/reference/issues.md` in the test title.** Fixing a
defect means deleting a `todo` flag in the same diff, which is the proof the fix landed.

`npm test` exiting 0 with todos listed is the correct state until those plans run.
