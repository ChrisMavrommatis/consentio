...
test('issue 14 - the modal identifies itself as a dialog', () => {
	assert.equal(mountModal().modal.getAttribute('role'), 'dialog');
});

test('issue 14 - the modal is marked aria-modal', () => {
	assert.equal(mountModal().modal.getAttribute('aria-modal'), 'true');
});

test('issue 14 - the modal has an accessible name', () => {
...
test('issue 14 - save and cancel are keyboard operable', () => {
	const { modal } = mountModal();
